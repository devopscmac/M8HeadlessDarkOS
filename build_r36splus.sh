...
# 7. Build SDL2 (required by EmulationStation)
source "${DARKOS_DIR}/build_sdl2.sh"

# 8. System utilities required for m8c operation
source "${DARKOS_DIR}/build_ogacontrols.sh"   # gamepad driver/controls for R36S Plus hardware
source "${DARKOS_DIR}/build_gptokeyb.sh"       # gamepad-to-keyboard mapping (used by m8c launcher)
source "${DARKOS_DIR}/build_emulationstation.sh" # main UI / launcher

# Emulators — commented out for M8-only build (saves ~10–14h build time)
# Uncomment any you want and rebuild; the package cache means only newly
# added items compile — everything else restores from cache in minutes.
